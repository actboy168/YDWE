...
fs::path WindowImplBase::GetZIPFileName() const
{
...
LRESULT WindowImplBase::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	m_PaintManager.SetResourcePath(m_PaintManager.GetInstancePath() / GetSkinFolder());

	if (GetResourceType() == UILIB_ZIP)
	{
		m_PaintManager.SetResourceZip(GetZIPFileName());
	}

	CControlUI* pRoot = builder.Create(GetSkinFile().c_str(), this, &m_PaintManager);
	ASSERT(pRoot);
