...
{
    if (m_pStrResourceZip == pStrPath) return;
    ...
    case WM_MOUSEHOVER:
        {
            ...
            std::wstring const& sToolTip = pHover->GetToolTip();
            if( sToolTip.empty() ) return true;
			m_Tooltip.Open(m_hInstance, m_hWndPaint, sToolTip.c_str(), pHover->GetPos());
        }
        return true;
    case WM_MOUSELEAVE:
        {
    ...
    case WM_MOUSEMOVE:
        {
            ...
            if( !m_bMouseTracking ) {
                ...
                tme.dwHoverTime = m_Tooltip.GetDelayTime();
...
{
	foreach (auto it, m_mOptionGroup) 
	{
		delete it.second;
	}
...
const CImageInfo* CPaintManagerUI::AddImage(const wchar_t* bitmap, HBITMAP hBitmap, int iWidth, int iHeight, bool bAlpha)
{
    ...
    CImageInfo* data = new CImageInfo(hBitmap, iWidth, iHeight, bAlpha);
    if (!m_mImageHash.Insert(bitmap, data)) 
	{
        delete data;
    }
...
{
	foreach (auto it, m_mImageHash) 
	{
		CImageInfo* data = it.second;
		if (data) 
		{
			delete data;
		}
...
{
    bool bRedraw = false;
    CImageInfo* pNewData = nullptr;

	foreach (auto it, m_mImageHash) 
	{
		std::wstring const& bitmap = it.first;
		CImageInfo* data = it.second;
		if (data != NULL)
		{
			pNewData = CImageInfo::Load(bitmap.c_str(), data->dwMask);
...
void CPaintManagerUI::AddDefaultAttribute(const wchar_t* pStrControlName, const wchar_t* pStrAttrKey, const wchar_t* pStrAttrValue)
{
	...
	if (pDefaultAttr)
	{
		pDefaultAttr->push_back(std::make_pair(pStrAttrKey, pStrAttrValue));
	}
	else
	{
		m_DefaultAttrHash[pStrControlName] = new CAttributesList();
		AddDefaultAttribute(pStrControlName, pStrAttrKey, pStrAttrValue);
	}
}
...
{
	foreach (auto it, m_DefaultAttrHash) 
	{
		delete it.second;
	}
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlFromNameHash(CControlUI* pThis, LPVOID pData)
{
    ...
    pManager->m_mNameHash[sName.c_str()] = pThis;
