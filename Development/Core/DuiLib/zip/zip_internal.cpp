...
namespace
{
	...
	uLong ReadZipBuffer(void* opaque, void* /*stream*/, void* buf, uLong size)
	{
		...
		if (!buffer || !buffer->data || !remaining_bytes)
			return 0;
		size = min(size, static_cast<uLong>(remaining_bytes));
	...
	long SeekZipBuffer(void* opaque, void* /*stream*/, uLong offset, int origin) 
	{
		...
		{
			buffer->offset = min(buffer->offset + static_cast<size_t>(offset),
				buffer->length);
			return 0;
		}
		...
		{
			buffer->offset = min(buffer->length, static_cast<size_t>(offset));
			return 0;
		}
		return -1;
	}
