...
namespace DuiLib
{
	...
	CTreeNodeUI::CTreeNodeUI( CTreeNodeUI* _ParentNode /*= NULL*/ )
	{
		...
		pCheckBox		= new CRadioButtonUI();
		pItemButton		= new CRadioButtonUI();
	...
	{
		if (nullptr == dynamic_cast<CTreeNodeUI*>(pControl))
			return false;
		...
		if(!pIndexNode && pTreeView && pTreeView->GetItemAt(GetTreeIndex()+1))
			pIndexNode = dynamic_cast<CTreeNodeUI*>(pTreeView->GetItemAt(GetTreeIndex()+1));
	...
	CTreeNodeUI* CTreeNodeUI::CalLocation( CTreeNodeUI* _pTreeNodeUI )
	{
		...
		_pTreeNodeUI->GetItemButton()->SetGroup(pItemButton->GetGroup().c_str());
