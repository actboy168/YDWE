...
namespace base {
	...
	static const char spell_dict[][7] = {
	...
	std::string chinese2pinyin(const std::string& chinese) {
		std::string pinyin;
		for (size_t j = 0; j < chinese.size();) {
			if (chinese[j] >= 0 && chinese[j] < 128) {
				pinyin += chinese[j];
				j++;
				continue;
			}
			int chrasc = chinese[j] * 256 + chinese[j + 1] + 256;
