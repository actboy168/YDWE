...
namespace DuiLib
{
	...
	{
		ForeachControl([=](CControlUI* pControl) -> bool
		{
	...
	void CContainerUI::EnableScrollBar(bool bEnableVertical, bool bEnableHorizontal)
	{
		if( bEnableVertical && !m_pVerticalScrollBar ) {
			...
			if (m_pManager) 
			{
				CAttributesList* pDefaultAttributes = m_pManager->GetDefaultAttributeList(_T("VScrollBar"));
				if (pDefaultAttributes) 
				{
					m_pVerticalScrollBar->ApplyAttributeTable(pDefaultAttributes);
				}
			}
		}
		...
		if( bEnableHorizontal && !m_pHorizontalScrollBar ) {
			...
			if (m_pManager) 
			{
				CAttributesList* pDefaultAttributes = m_pManager->GetDefaultAttributeList(_T("HScrollBar"));
				if (pDefaultAttributes) 
				{
					m_pHorizontalScrollBar->ApplyAttributeTable(pDefaultAttributes);
				}
			}
		}
	...
	int CContainerUI::FindSelectable(int iIndex, bool bForward /*= true*/) const
	{
		...
		if( bForward ) {
			for( int i = iIndex; i < GetCount(); i++ ) {
				if (dynamic_cast<IListItemUI*>(GetItemAt(i)) != nullptr
	...
	bool CContainerUI::ForeachControl(std::function<bool(CControlUI*)> Proc)
	{
		if (m_pVerticalScrollBar) 
		{
			if (!m_pVerticalScrollBar->ForeachControl(Proc)) return false;
			if (!Proc(m_pVerticalScrollBar)) return false;
		}
		if (m_pHorizontalScrollBar)
		{
			if (!m_pHorizontalScrollBar->ForeachControl(Proc)) return false;
			if (!Proc(m_pHorizontalScrollBar)) return false;
		}

		for (int it = 0; it < m_items.GetSize(); ++it)
		{
			if (!static_cast<CControlUI*>(m_items[it])->ForeachControl(Proc)) return false;
			if (!Proc(static_cast<CControlUI*>(m_items[it]))) return false;
		}
